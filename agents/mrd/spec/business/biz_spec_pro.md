...
### 10.1 업종별 표준 지표와 비교하여 사업의 건강도를 평가합니다.

| 지표명               | 산출 공식                      | 업종별 검토 기준 (우수)                                                      |
| :------------------- | :----------------------------- | :--------------------------------------------------------------------------- |
...
| **1인당 월매출액**   | `월매출액 / (종업원수 / 12)`   | 제조(1,000만), 소매(2,280만), 외식(900만), 시설(720만), 기술(450만) **이상** |
...
## 10.2 세부 검증 규칙 (Validation Rules)

### 10.2.1 데이터 매핑 가이드

- **총투자금액**: `invest.production`과 `invest.sales`의 모든 `investPrice` 합계.
- **종업원수**: `job.production` 및 `job.sales` 배열 내 `jobCnt` 필드의 전체 합산.
- **자기자본**: `loan.equity` 합계 + 누적 이익잉여금(경상이익의 합).
- **총자산**: `비유동자산(투자금-상각액) + 유동자산`.

### 10.2.2 보정 로직 검증 (연간 -> 월간)

에이전트는 다음 두 지표 계산 시 **분모를 12로 나누는 보정 로직**이 코드에 포함되어 있는지 반드시 확인해야 합니다.

1. **총자산영업이익률**: `영업이익 / (투자금액 / 12)`
2. **1인당 월매출액**: `월매출액 / (종업원수 / 12)`
...
- business_spec_expert.md 를 참조한다.
