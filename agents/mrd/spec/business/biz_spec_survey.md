...
9. 설문 상태 전환 및 문항 수정 제한 정책
   설문 응답 데이터의 무결성을 보호하기 위해, 응답이 수집된 이후의 문항 수정은 엄격히 제한하며 데이터 초기화를 전제로만 허용합니다.

9-1. 상태별 편집 권한 정의
설문 상태에 따라 시스템에서 허용하는 편집 범위는 다음과 같습니다.
설문 상태,문항 구조 수정,텍스트(오타) 수정,비고
작성중 (DRAFT),허용,허용,데이터가 없는 경우에만 자유롭게 수정 가능
수집중 (OPEN),차단,차단,편집 화면 진입 시 모든 입력창/버튼 Read-only
마감 (CLOSED),차단,차단,모든 편집 기능 비활성화

9-2. 문항 수정을 위한 [시나리오 B] 프로세스
관리자가 문항을 반드시 고쳐야 한다면, 다음의 '강제 초기화' 단계를 거쳐야 합니다.

수정 시도: 관리자가 '수집중' 상태에서 [문항 수정] 버튼을 클릭합니다.

데이터 존재 확인: 시스템이 tb_results를 카운트하여 응답이 있는지 확인합니다.

결정적 팝업(Modal) 노출:

상황: 응답 데이터가 1건이라도 있는 경우.

메시지: "이미 수집된 응답 데이터가 존재하여 문항을 수정할 수 없습니다. 문항을 수정하시려면 기존 응답을 모두 삭제하고 작성중 상태로 돌아가야 합니다."

버튼: [취소] / [데이터 삭제 후 수정하기]

데이터 파기 및 상태 회귀:

관리자가 승인하면 tb_results, tb_results_label의 해당 SURVEY_ID 데이터를 모두 삭제합니다.

설문 상태를 DRAFT로 변경합니다.

이제 데이터가 0건이므로 모든 문항 편집 기능(추가/삭제/유형변경)이 활성화됩니다.

9-3. 시스템 제어 세부 사항 (UI/UX)
수집 중 편집 차단: 설문 상태가 OPEN일 때 질문 탭(Questions)에 들어가면, 모든 input, select, button 요소에 disabled 속성을 부여하여 시각적으로나 기능적으로 수정을 차단합니다.
